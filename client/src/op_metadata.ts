...
export class OPMetadata {
    ...
    constructor() {
        ...
        if (existsSync(`./src/op_generated.ts`)) {
            unlinkSync(`./src/op_generated.ts`);
        }
        ...
        for (const key of keys) {
            ...
            appendFileSync(`./src/op_generated.ts`, result);
        }
    ...
    MakeMetadata() {
        ...
        for (const file of files) {
            // if (file.toLowerCase().startsWith('opw00001') === false) continue;
    ...
    MakeTSCode(item: OPMetadataItemType) {
        let result = '';
        result += `export type OPInput${item.tr_code} = {\n`;
        for (const input of item.input) {
            result += `    '${input.name}${input.fid === -1 ? '' : `_${input.fid}`}': string;\n`;
        }
        result += `};\n`;

        if (item.output_multi.length > 0) {
            result += `export type OPOutput${item.tr_code}MultiItem = {\n`;
            for (const output of item.output_multi) {
                result += `    '${output.name}${output.fid === -1 ? '' : `_${output.fid}`}': string;\n`;
            }
            result += `};\n`;
        }

        result += `export type OPOutput${item.tr_code} = {\n`;
        for (const output of item.output_single) {
            if (item.tr_code === 'OPW20009' && output.offset === 126) continue;//예외처리
            result += `    '${output.name}${output.fid === -1 ? '' : `_${output.fid}`}': string;\n`;
        }
        if (item.output_multi.length > 0) {
            result += `    multi_items: OPOutput${item.tr_code}MultiItem[];\n`;
        }
        result += `};\n`;
